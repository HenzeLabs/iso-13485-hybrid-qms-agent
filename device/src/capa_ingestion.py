...
class CAPAIngestion:
    ...
    def update_capa_analysis(
        ...
        Security: Uses parameterized queries to prevent SQL injection (VULN-001 fix)
        """
        ...
        if root_cause is not None:
            update_parts.append("root_cause = @root_cause")
            query_params.append(bigquery.ScalarQueryParameter("root_cause", "STRING", root_cause))
        if correction is not None:
            update_parts.append("correction = @correction")
            query_params.append(bigquery.ScalarQueryParameter("correction", "STRING", correction))
        if corrective_action is not None:
            update_parts.append("corrective_action = @corrective_action")
            query_params.append(bigquery.ScalarQueryParameter("corrective_action", "STRING", corrective_action))
        if preventive_action is not None:
            update_parts.append("preventive_action = @preventive_action")
            query_params.append(bigquery.ScalarQueryParameter("preventive_action", "STRING", preventive_action))
        ...
        # Use parameterized query for capa_id (VULN-001 fix)
        query_params.append(bigquery.ScalarQueryParameter("capa_id", "STRING", capa_id))
        ...
        query_job = self.bq_client.client.query(sql, job_config=job_config)
        query_job.result()
        return True
    ...
    def add_capa_approval(
        ...
        approval_data = [{
            ...
            "approval_date": datetime.now(UTC).isoformat() if approval_status != "Pending" else None,
            "comments": comments
        }]
    ...
    def get_capa_details(self, capa_id: str) -> Dict:
        ...
        Security: Uses parameterized queries to prevent SQL injection (VULN-001 fix)
        """
        # Get CAPA case - parameterized query
        case_sql = f"""
        SELECT *
        ...
        LIMIT 1
        """
        job_config_case = bigquery.QueryJobConfig(
            query_parameters=[
                bigquery.ScalarQueryParameter("capa_id", "STRING", capa_id)
            ]
        )
        case = self.bq_client.query(case_sql, job_config_case)

        # Get actions - parameterized query
        actions_sql = f"""
        SELECT *
        ...
        ORDER BY due_date
        """
        job_config_actions = bigquery.QueryJobConfig(
            query_parameters=[
                bigquery.ScalarQueryParameter("capa_id", "STRING", capa_id)
            ]
        )
        actions = self.bq_client.query(actions_sql, job_config_actions)

        # Get approvals - parameterized query
        approvals_sql = f"""
        SELECT *
        ...
        ORDER BY created_at
        """
        job_config_approvals = bigquery.QueryJobConfig(
            query_parameters=[
                bigquery.ScalarQueryParameter("capa_id", "STRING", capa_id)
            ]
        )
        approvals = self.bq_client.query(approvals_sql, job_config_approvals)
