...
export class LLMAssistant {
  ...
  async sendMessage(message: string, userId: string, sessionId?: string): Promise<AssistantMessage> {
    ...
    try {
      ...
      if (assistantResponse.function_call) {
        return await this.handleFunctionCall(assistantResponse, userId, sessionId);
      }
      ...
      const assistantMessage: AssistantMessage = {
        id: Date.now().toString(),
        role: 'assistant',
        content: assistantResponse.content || 'I apologize, but I couldn\'t generate a response.',
        timestamp: new Date().toISOString()
      };
  ...
  private async handleFunctionCall(response: any, userId: string, sessionId?: string): Promise<AssistantMessage> {
  ...
  async confirmPendingAction(confirmationId: string, confirmed: boolean, userId: string, sessionId?: string): Promise<AssistantMessage> {
    const pending = this.conversation.pending_confirmations.find(p => p.id === confirmationId);
    if (!pending) {
      throw new Error('Confirmation not found');
    }

    // Remove from pending
    this.conversation.pending_confirmations = this.conversation.pending_confirmations.filter(p => p.id !== confirmationId);
    ...
    const confirmedCall = { ...pending.function_call, confirmed: true, session_id: sessionId };
